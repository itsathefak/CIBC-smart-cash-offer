  ...
  <section class="investing-activity">
    <div class="container">
      ...
      <div class="activity-container">
        ...
        <p>
          Try your hand at investing with our virtual stock market simulator.
          Make investment decisions and see how your portfolio performs over
          time.
        </p>
        <div class="simulation-area">
          ...
          <button @click="makeInvestment" class="btn btn-primary">
            Invest
          </button>
        </div>
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="investment in portfolio" :key="investment.symbol">
                ...
                <td>
                  <button
                    @click="removeInvestment(investment.symbol)"
        ...
        <button @click="simulateMarketChanges" class="btn btn-secondary mt-3">
...
const makeInvestment = () => {
  if (selectedStock.value && investmentAmount.value > 0) {
    ...
    const existingInvestment = portfolio.value.find(
      (inv) => inv.symbol === selectedStock.value.symbol
    );

    if (existingInvestment) {
      existingInvestment.shares += shares;
      existingInvestment.totalInvested += investmentAmount.value;
      // Update profitLoss safely after reinvesting
      existingInvestment.profitLoss =
        existingInvestment.shares * existingInvestment.currentPrice -
        existingInvestment.totalInvested;
    } else {
      portfolio.value.push({
        ...
        profitLoss: 0, // ✅ Set initial value to avoid undefined
      });
    }

    investmentAmount.value = 1000; // Reset after investment
  }
};

const removeInvestment = (symbol) => {
  portfolio.value = portfolio.value.filter((inv) => inv.symbol !== symbol);
};

const totalPortfolioValue = computed(() => {
  return portfolio.value.reduce(
    (total, inv) => total + inv.shares * inv.currentPrice,
    0
  );
});
...
const totalProfitLoss = computed(() => {
  return portfolio.value.reduce(
    (total, inv) => total + (inv.shares * inv.currentPrice - inv.totalInvested),
    0
  );
});
...
const simulateMarketChanges = () => {
  ...
  portfolio.value.forEach((investment) => {
    const stock = availableStocks.value.find(
      (s) => s.symbol === investment.symbol
    );
    if (stock) {
      investment.currentPrice = stock.price;
      investment.profitLoss =
        investment.shares * investment.currentPrice - investment.totalInvested;
    }
  });
};
</script>
